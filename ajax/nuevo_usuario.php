...
if (empty($_POST['firstname'])) {
...
} elseif (!preg_match('/^[a-zA-Z\s]{1,25}$/', $_POST['firstname'])) {
    $errors[] = "El nombre contiene caracteres especiales o excede los 25 caracteres";
} elseif (empty($_POST['lastname'])) {
    $errors[] = "Apellidos vacíos";
} elseif (!preg_match('/^[a-zA-Z\s]{1,25}$/', $_POST['lastname'])) {
    $errors[] = "El apellido contiene caracteres especiales o excede los 25 caracteres";
    ...
    $errors[] = "Nombre de usuario no encaja en el esquema de nombre: Solo letras y números permitidos, de 2 a 64 caracteres";
} elseif (empty($_POST['user_email'])) {
    $errors[] = "El correo electrónico no puede estar vacío";
} elseif (strlen($_POST['user_email']) > 64) {
    $errors[] = "El correo electrónico no puede ser superior a 64 caracteres";
} elseif (!filter_var($_POST['user_email'], FILTER_VALIDATE_EMAIL)) {
    $errors[] = "Su dirección de correo electrónico no está en un formato de correo electrónico válido";
} elseif (
    ...
    && preg_match('/^[a-zA-Z\s]{1,25}$/', $_POST['firstname'])
    && preg_match('/^[a-zA-Z\s]{1,25}$/', $_POST['lastname'])
